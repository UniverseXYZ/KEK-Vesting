...
async function main () {
    ...
    for (const key in owners) {
        ...
        currentVesting = await Vesting.deploy(owners[key].address, _xyzTokenAddress, startTime, amount)
        await currentVesting.deployed()

        await XYZToken.transfer(currentVesting.address, amount)

        console.log(`Deploy vesting contract no. ${counter} for ${owners[key].address} at ${currentVesting.address}`)
        counter++;
    }
}
