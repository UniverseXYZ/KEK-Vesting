...
async function main () {
    ...
    for (let i = startIndex; i < owners.length; i++) {
        ...
        currentVesting = await Vesting.deploy(owner.address, _xyzTokenAddress, startTime, amount, {gasPrice: 20_000_000_000})
        await currentVesting.deployed()

        await XYZToken.transfer(currentVesting.address, amount)

        console.log(`Deploy vesting contract index. ${i} for ${owner.address} at ${currentVesting.address}, amount ${owner.amount}`)
    }
}
